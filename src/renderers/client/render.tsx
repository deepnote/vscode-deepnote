...
export class CellOutput extends React.Component<ICellOutputProps> {
    ...
    private renderImage(mimeType: string, data: Blob | MediaSource, metadata: Record<string, unknown> = {}) {
        if (this.props.ctx.postMessage) {
            this.props.ctx.postMessage({ type: 'isDeepnoteExtensionInstalled' } as IsDeepnoteExtensionInstalled);
        }
        if (this.props.ctx.onDidReceiveMessage) {
            const disposable = this.props.ctx.onDidReceiveMessage((response: Partial<IsDeepnoteExtensionInstalled>) => {
                if (response?.type === 'isDeepnoteExtensionInstalled' && response.response) {
                    (globalThis as any).__isDeepnoteInstalled = response.response;
                }
            });
        ...
        const copyPlotImage = () => {
            void writeImageToClipboard(data as Blob);
        };
        const onMouseOver = () => {
            if (!(globalThis as any).__isDeepnoteInstalled) {
                return;
            }
            // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
